...
class ProcessModelOutput():
	...
	def read_output_from_SWOLF (self, process_name, filepath):  #excel file
		...
		technosphere = (data[[2]+np.arange(33,68).tolist()][8:69]).to_dict()
		biosphere =  (data[np.arange(0,63).tolist()][71:1826]).to_dict()
		...
		for x in waste_fractions[1:]:
			VV[x] = {}
			for y in TT:
				VV[x][y] = self.check_nan(waste[y][x])
		waste=VV
		
		
		# revising the technosphere dictionary. technosphere[key1][key2] , key1: waste fraction , key2: technosphere stream
		EE=[str(i) + " : "+technosphere[i][8] for i in ([2]+np.arange(33,68).tolist())]  # name of the prodcuts _ waste
		...
		for x in waste_fractions[1:]:
			VV[x] = {}
			for y in CC:
				VV[x][y] = self.check_nan(technosphere[y][x])
		technosphere=VV
		...
		for z in waste_fractions[1:]:
			...
			for oo in biosphere_keys.values():
				biosphere[z][oo[0]]=biosphere[z].pop(pp)
				pp += 1   
