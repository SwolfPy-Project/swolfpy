...
# This imports equivalent functions defined in Excel.
...
class Spreadsheet(object):
    ...
    def set_value(self, address, val):
        ...
        try:
            if address in self.named_ranges:
                ...
                cmap = self.cellmap[ref_address]
                if cmap.is_range:           
                    cells_to_set = []
                    if not isinstance(val, list):
                        val = [val] * len(cells_to_set)
                    for child in cmap.range.cells:
                        self.reset(child)                
                    cmap.range.values = val
            ...
            # case where the address refers to a single value
                ...
                if address in self.named_ranges:  # if the cell is a named range, we need to update and fix the reference cell
                    ...
                    self.reset(ref_cell) #reset when using named_range
                    ref_cell.value = val
    ...
    def evaluate(self,cell,is_addr=True):
        ...
        if cell in self.named_ranges:
            ref_address = self.named_ranges[cell]
            cell = self.cellmap[ref_address]
        ...
        try:
            if cell.is_range:	
                test = self.evaluate(cell.range.cells[0].address())
                if isinstance(test,list):
                    for i,j in enumerate(cell.range.cells):				
                        self.set_value(str(j),test[i])
                    for child in cell.range.cells:
            ...
            elif cell in self.named_ranges:
                ref_address = self.named_ranges[cell]
                for child in ref_address.range.cells:
                    self.evaluate(child.address())
